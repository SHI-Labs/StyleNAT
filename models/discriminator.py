...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        for i in range(log_size, 2, -1):
            ...
            self.from_rgbs.append(FromRGB(in_channel, downsample=i != log_size, sn=args.sn))
            self.convs.append(ConvBlock(in_channel, out_channel, args.blur_kernel, sn=args.sn))

            in_channel = out_channel

        self.from_rgbs.append(FromRGB(channels[4], sn=args.sn))

        self.stddev_group = 4
        self.stddev_feat = 1

        self.final_conv = ConvLayer(in_channel + 1, channels[4], 3, sn=args.sn)
        if args.sn:
