...
The code was cloned from StyleSwin's implementation (relies on the above) which
introduces Swin layers.
We introduce the NAT layers and provide options such that we can use Swin, NA,
or transformers. This is to allow for comparisons to be made by other
researchers and to encourage a more open testing environment.
'''
import math
import numpy as np
...
class Transformer(nn.Module):
    def __init__(self,
                 ...
                 drop_path=0.,
                 act_layer=nn.GELU,
                 norm_layer=AdaptiveInstanceNorm,
                 style_dim=512,
                 **kwargs,
                 ):
        super().__init__()
        self.dim = dim
        self.attn = MHSA(dim=dim, num_heads=num_heads, attn_drop=attn_drop, proj_drop=drop)
        self.norm1 = norm_layer(dim, style_dim)
        self.norm2 = norm_layer(dim, style_dim)
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class Generator(nn.Module):
    def __init__(
        ...
        for _ in range(args.n_mlp):
            layers.append(
                EqualLinear(
                    self.style_dim, self.style_dim, lr_mul=args.lr_mlp, activation='fused_lrelu'
                )
            )
        ...
        in_channels = [
            ...
            512,
            512,
            256 * args.channel_multiplier,
            128 * args.channel_multiplier,
            64 *  args.channel_multiplier,
